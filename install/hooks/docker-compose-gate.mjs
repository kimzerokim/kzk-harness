#!/usr/bin/env node
// docker-compose-gate.mjs — kzk-harness Gate 3.5 (propagation copy).
//
...
//   ~/.claude/skills/.kzk-harness-shared/hooks/docker-compose-gate.mjs
//
// REPO_ROOT is resolved from CWD / PWD (the user's project root at hook fire time),
// with git rev-parse fallback — same pattern as check-cycle-exit.mjs.
//
...
// ---------------------------------------------------------------------------
// Shared marker regions (9 total — byte-equal with .claude/hooks copy)
// ---------------------------------------------------------------------------
