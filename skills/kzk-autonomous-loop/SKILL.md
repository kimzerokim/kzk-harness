...
8. **Edit/Write 1× FAIL → halt** (Cycle 50 incident, external-project session). Tool failure does NOT trigger polite halt. Per `kzk-tool-retry §Auto-retry`, single failure = same-turn re-read + retry. Halt threshold is **2 consecutive failures on the same file**, then queue Q-TOOL and proceed to next task. PostToolUse hook `edit-failure-retry.mjs` injects a forcing system-reminder on failure detection — agent cannot silently halt. cross-ref: `kzk-tool-retry §Forcing mechanism`.
