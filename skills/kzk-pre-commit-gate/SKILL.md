...
After Gate 0 passes, run `mcp__plugin_oh-my-claudecode_t__deepinit_manifest action=save` once at the END of the commit batch (autonomous run) or at PR-creation time (interactive). Manifest baseline file is gitignored (`.omc/deepinit-manifest.json`); it just lets the next session's `action=diff` produce a real signal.
...
If the commit touches **no** source code — only docs/configs/screenshots (`*.md`, `docs/**`, `CLAUDE.md`, `DESIGN.md`, `harness-flow-progress.md`, `.claude/skills/**/*.md`, `docs/screenshots/**`) — then:
