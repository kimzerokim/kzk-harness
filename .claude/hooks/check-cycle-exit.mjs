#!/usr/bin/env node
// check-cycle-exit.mjs — kzk-harness PROJECT-LOCAL PreToolUse hook.
//
...
//
// Scope: THIS REPO + propagated globally by install-global.sh (default ON).
// Registered in .claude/settings.json (PreToolUse Bash matcher, after check-skill-flow-fresh.mjs).
//
// Bypass env vars:
...
function extractCommitMessage(command) {
  ...
  // Fall back to .git/COMMIT_EDITMSG (interactive commit — file written before PreToolUse fires)
  try {
...
// stripQuotedAndHeredoc and parseInlineEnv imported from ./lib/cycle-exit-utils.mjs
